...
FUNDAMENTALS_SCRIPT = os.path.join(OPTIONPLAY_ROOT, "scripts", "populate_fundamentals.py")
...
async def get_db_coverage():
    ...
    try:
        ...
        # Options
        r = conn.execute("SELECT MAX(quote_date), COUNT(*), COUNT(DISTINCT underlying) FROM options_prices").fetchone()
        ...
        # OHLCV
        r = conn.execute("SELECT MAX(quote_date), COUNT(*), COUNT(DISTINCT symbol) FROM daily_prices").fetchone()
        ohlcv_last, ohlcv_rows, ohlcv_symbols = r
        ohlcv_days = days_stale(ohlcv_last)

        # Fundamentals
        r = conn.execute("SELECT MAX(updated_at), COUNT(*) FROM symbol_fundamentals WHERE delisted = 0").fetchone()
        fund_last_raw, fund_rows = r
        fund_last = fund_last_raw[:10] if fund_last_raw else None
        fund_days = days_stale(fund_last)

        # Earnings
        r = conn.execute("SELECT MAX(earnings_date), COUNT(*), COUNT(DISTINCT symbol) FROM earnings_history").fetchone()
        ...
        return {
            ...
            "tables": {
                "vix": {
                    "last_date": vix_last, "days_stale": vix_days,
                    "badge": badge(vix_days), "row_count": vix_rows,
                },
                "options": {
                    "last_date": opt_last, "days_stale": opt_days,
                    "badge": badge(opt_days), "row_count": opt_rows,
                    "symbol_count": opt_symbols, "greeks_count": greeks_rows,
                },
                "ohlcv": {
                    "last_date": ohlcv_last, "days_stale": ohlcv_days,
                    "badge": badge(ohlcv_days), "row_count": ohlcv_rows,
                    "symbol_count": ohlcv_symbols,
                },
                "fundamentals": {
                    "last_date": fund_last, "days_stale": fund_days,
                    "badge": badge(fund_days), "row_count": fund_rows,
                },
                "earnings": {
                    "last_date": earn_max, "row_count": earn_rows,
                    "symbol_count": earn_symbols,
                    "badge": "green" if earn_future_symbols and earn_future_symbols > 0 else "amber",
...
async def run_fundamentals_update(payload: dict = Body(default={})):
    """Run populate_fundamentals.py with optional mode selection."""
    if not os.path.exists(FUNDAMENTALS_SCRIPT):
        raise HTTPException(status_code=404, detail="populate_fundamentals.py not found")
    ...
    try:
        result = subprocess.run(
            ...
            timeout=600, cwd=OPTIONPLAY_ROOT,
        )
        return {
            "status": "completed" if result.returncode == 0 else "error",
            "returncode": result.returncode,
        ...
        return {"status": "timeout", "message": "Fundamentals update timed out after 10 minutes"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
